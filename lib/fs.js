...
const ReadStream = Stream.extend({
  constructor: function ReadStream(path, options) {
    ...
    try {
      ...
      let pump = new StreamPump(source, this.length, this.position, 0, 0, true);
      _(this).request = pump;
      this.read();
    } catch (error) {
      this.readable = false;
...
const WriteStream = Stream.extend({
  ...
  flags: 'w',
  mode: FILE_PERMISSION,
  write: function write(content, encoding) {
    let { fd, observer } = _(this);
    let copier = _(this).request = new StreamCopier();
    let target = new FileOutputStream();
    target.init(fd, FLAGS(this.flags), Mode(this.mode), target.DEFER_OPEN);
  ...
  flush: function flush() {
    // TODO: I guess we don't have to write to stream unless flush is called ?
  },
...
exports.read = function read(fd, buffer, offset, length, position, callback) {
  ...
  let bytesRead = 0;
  let [ value, mode, flags ] = _(fd);
  let readStream = new ReadStream(value, {
    mode: mode,
    flags: flags,
    length: length,
    position: position
  ...
  readStream.on("data", function onData(chunck) {
    buffer.copy(buffer, offset + bytesRead);
    bytesRead += buffer.length;
  });
  readStream.on("end", function onEnd(callback) {
